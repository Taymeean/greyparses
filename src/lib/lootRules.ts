...
export function isItemAllowedForClass(
  ...
  const isTier = item.type === 'TIER_SET'
...
export async function validateSRChoice(
  ...
  | { ok: false; code: 'PLAYER_NOT_FOUND' | 'ITEM_NOT_FOUND' | 'INVALID_ITEM' | 'INVALID_BOSS'; message: string }
> {
  ...
  if (!item) return { ok: false, code: 'ITEM_NOT_FOUND', message: 'Loot item not found.' };

  const { allowed, isTier } = isItemAllowedForClass(player.class, item);
  if (!allowed) {
    return { ok: false, code: 'INVALID_ITEM', message: 'Selected item is not valid for this class.' };
  }
  ...
  if (bossId) {
    ...
    if (!link) {
      return { ok: false, code: 'INVALID_BOSS', message: 'Selected boss does not drop this item.' };
    }
  }
