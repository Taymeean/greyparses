...
type ClassInfo = { id: number; name: string; armorType: string; tierPrefix: string };
...
type Me = { authenticated: boolean; session?: { playerId: number; name: string; role: string } };
...
export default function SRPage() {
    ...
    (async () => {
      try {
        ...
        if (meJ.authenticated && srJ.rows) {
          const mine = srJ.rows.find(r => r.playerId === meJ.session?.playerId);
          if (mine) {
            setEditItemId(mine.choice?.lootItem?.id ?? '');
            setEditBossId(mine.choice?.boss?.id ?? '');
            setEditNotes(mine.choice?.notes ?? '');
  ...
  // rows to render (hide inactive)
  const rowsForRender = useMemo(() => {
    if (!week) return [];
    return week.rows.filter(r => r.active !== false);
  }, [week]);
  ...
  useEffect(() => {
    ...
    async function refreshBossOptions() {
      ...
      try {
        const res = await fetch(`/api/loot/${editItemId}/bosses`, { cache: 'no-store' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-neutral-400">{week ? week.label : '—'}</div>
      </div>
      ...
      {!me?.authenticated && (
        <div className="p-3 border rounded border-neutral-800 bg-amber-50/10 text-amber-200 text-sm">
          ...
          <a className="underline" href="/claim">/claim</a> to claim your character.
        </div>
      )}

      {myRow && locked && (
        <div className="p-3 border rounded border-neutral-800 bg-neutral-900 text-neutral-200 text-sm">
          Your SR is currently <b>locked</b>. You can’t change it until officers unlock your row (or the boss dies and they unlock others).
        </div>
      )}
          ...
          <table className="min-w-full text-[15px]">
            ...
            <tbody>
              {rowsForRender.map(r => {
                ...
                return (
                  <tr key={r.playerId}>
                    ...
                    {/* Updated */}
                    <td className="whitespace-nowrap">{formatNY(c?.updatedAt)}</td>
              ...
              {rowsForRender.length === 0 && (
                <tr>
                  <td colSpan={10} className="text-center text-neutral-400 py-8">No players</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <p className="text-xs text-neutral-400">
        Boss list filters to only bosses that drop the selected item. If there’s exactly one, it auto-selects.
      </p>
    </div>
  );
}
