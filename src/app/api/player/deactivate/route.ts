...
export async function POST(req: Request) {
  ...
  if (confirmName.trim().toLowerCase() !== session.name.toLowerCase()) {
    return NextResponse.json({ error: 'Confirmation name does not match your character' }, { status: 400 });
  }

  const player = await prisma.player.findUnique({ where: { id: session.playerId }, select: { id: true, active: true } });
